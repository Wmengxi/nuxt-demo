<template>
  <div class="nav-box">
    ...
    <div :class="['nav-item', (isCurrent('news') || isCurrent('news-newsinfo')) ? 'selected' : '']" @click="navTo('news')">新闻资讯</div>
    <div class="nav-item">服务介绍</div>
    <div :class="['nav-item', isCurrent('lovestory') ? 'selected' : '']" @click="navTo('lovestory')">晒幸福</div>
    <div :class="['nav-item', (isCurrent('findpartner') || isCurrent('findpartner-partnerinfo')) ? 'selected' : '']" @click="navTo('findpartner')">寻找对象</div>
